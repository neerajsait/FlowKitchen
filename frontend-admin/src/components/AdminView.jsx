...
export default function AdminView({ onLogout, dbMode }) {
  ...
  const [outletOwnerId, setOutletOwnerId] = useState("");
  ...
  const handleAddOutlet = async (e) => {
    ...
    try {
      ...
      const data = {
        ...
        owner_id: outletOwnerId ? parseInt(outletOwnerId) : null
      };
      ...
      setOutletName(""); setOutletAddress(""); setOutletLatitude(""); setOutletLongitude(""); setOutletRevenueShare(""); setOutletOwnerId("");
      loadData();
    } catch (err) { showToast("Failed: " + err.message, "error"); }
  };
  ...
  const openEditOutlet = (outlet) => {
    ...
    setOutletOwnerId(outlet.owner_id || "");
  ...
  const handleAddStaff = async (e) => {
    e.preventDefault();
    try {
      let finalEmail = staffEmail;
      let finalPassword = staffPassword;
      if (staffRole === "staff" || staffRole === "kitchen") {
        if (!finalEmail) finalEmail = `${staffPhone}@staff.foodpilot.local`;
        if (!finalPassword) finalPassword = "AutoGeneratedPass123!";
      }
      ...
      if (editingUserId) {
        ...
        if (!finalPassword) throw new Error("Password is required for new accounts");
...
{ id: "admin_logs", label: "Admin Audit", icon: FileText, depts: ["SuperAdmin"] },
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-base)" }}>
          ...
          <div className="grid-responsive-15fr" style={{ gap: "1.5rem" }}>
            <div className="panel" style={{ padding: "1.5rem" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" }}>
                ...
                <button className="btn btn-secondary" style={{ padding: "0.35rem 0.75rem", fontSize: "0.78rem" }} onClick={() => setActiveTab("customer_orders")}>
      ...
      {/* Add Outlet */}
      <Modal open={showAddOutlet} onClose={() => { setShowAddOutlet(false); setEditingOutletId(null); setOutletName(""); setOutletAddress(""); setOutletLatitude(""); setOutletLongitude(""); setOutletRevenueShare(""); setOutletOwnerId(""); }} title={editingOutletId ? "Edit Outlet" : "Register Outlet"} width={520}>
        <form onSubmit={handleAddOutlet} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div className="grid-responsive-2col" style={{ gap: "1rem" }}>
            ...
            <div className="form-group" style={{ margin: 0, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
              <label className="form-label">Assign Outlet Owner</label>
