...
export default function OutletOwnerView({ onLogout, dbMode }) {
  ...
  const [editStaffForm, setEditStaffForm] = useState({ full_name: "", phone: "", is_active: true, outlet_id: "" });
  ...
  const [addStaffForm, setAddStaffForm] = useState({ full_name: "", phone: "", pin: "", role: "staff", outlet_id: "" });
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      const payload = {
        ...addStaffForm,
        email: `${addStaffForm.phone}@staff.foodpilot.local`,
        password: "AutoGeneratedPass123!"
      };
      await api.adminCreateUser(payload);
      showToast("Staff added successfully!", "success");
      setShowAddStaff(false);
      setAddStaffForm({ full_name: "", phone: "", pin: "", role: "staff", outlet_id: "" });
      loadData();
    } catch (err) {
  ...
  const lookupCoordinates = async (address, type) => {
    if (!address.trim()) {
      showToast("Please enter an address first", "success");
      return;
    }
    if (type === "add") {
  ...
  const handleSelectOutlet = async (o) => {
    try {
      ...
      showToast("Failed to load live stock: " + err.message, "error");
    ...
    <div className="animate-fade-in" style={{ padding: "1rem", maxWidth: "100%", margin: "0" }}>
      ...
      <div style={{
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          <h1 style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(1.2rem, 3vw, 1.5rem)", fontWeight: 900, margin: 0, letterSpacing: "-0.02em" }}>Outlet Owner Dashboard</h1>
          <p style={{ opacity: 0.85, fontSize: "clamp(0.75rem, 2vw, 0.85rem)", margin: "0.25rem 0 0", fontWeight: 500 }}>Monitor live stock levels, configure retail stations, and audit safety limits</p>
        </div>
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", flexWrap: "wrap", position: "relative", zIndex: 2 }}>
          ...
          <button className="btn" onClick={openProfileModal} style={{
            ...
            padding: "0.4rem 0.8rem", borderRadius: "8px", fontWeight: "700", display: "flex", alignItems: "center", gap: "0.3rem", fontSize: "0.85rem",
            cursor: "pointer", transition: "all 0.2s"
          }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
        <div className="stat-card" style={{
          background: "var(--bg-elevated)", border: "1px solid var(--border-light)", borderRadius: "12px", padding: "1rem",
          boxShadow: "0 5px 15px rgba(0,0,0,0.02)", display: "flex", flexDirection: "column", gap: "0.4rem",
        ...
        onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 5px 15px rgba(0,0,0,0.02)"; }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Managed Outlets</span>
            <div style={{ width: 32, height: 32, borderRadius: "10px", background: "rgba(67, 160, 71, 0.08)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--brand)" }}><Store size={16} /></div>
          </div>
          <div style={{ fontSize: "1.8rem", fontWeight: 900, color: "var(--text-primary)", fontFamily: "var(--font-heading)" }}>{outlets.length}</div>
          <div style={{ fontSize: "0.7rem", color: "var(--text-muted)" }}>Assigned under ownership</div>
        </div>
        ...
        onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 5px 15px rgba(0,0,0,0.02)"; }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Restock Alerts</span>
            <div style={{ width: 32, height: 32, borderRadius: "10px", background: lowStockCount > 0 ? "rgba(239, 68, 68, 0.08)" : "rgba(16, 185, 129, 0.08)", display: "flex", alignItems: "center", justifyContent: "center", color: lowStockCount > 0 ? "var(--error)" : "var(--success)" }}><AlertTriangle size={16} /></div>
          </div>
          <div style={{ fontSize: "1.8rem", fontWeight: 900, color: lowStockCount > 0 ? "var(--error)" : "var(--success)", fontFamily: "var(--font-heading)" }}>{lowStockCount}</div>
          <div style={{ fontSize: "0.7rem", color: "var(--text-muted)" }}>{lowStockCount > 0 ? "Action required immediately" : "All outlet stocks are healthy"}</div>
        </div>
        ...
        onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 5px 15px rgba(0,0,0,0.02)"; }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Total Unit Stock</span>
            <div style={{ width: 32, height: 32, borderRadius: "10px", background: "rgba(141, 78, 39, 0.08)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--accent-terracotta)" }}><Package size={16} /></div>
          </div>
          <div style={{ fontSize: "1.8rem", fontWeight: 900, color: "var(--text-primary)", fontFamily: "var(--font-heading)" }}>{totalStock}</div>
        </div>
      </div>

      <div style={{ display: "flex", gap: "1.5rem", alignItems: "center", marginTop: "1.5rem", marginBottom: "1rem", borderBottom: "1px solid var(--border-light)" }}>
        {["inventory", "staff"].map(t => (
          <button key={t} onClick={() => setActiveTab(t)} style={{
            ...
            borderBottom: activeTab === t ? "2px solid var(--brand)" : "2px solid transparent", paddingBottom: "0.5rem",
            marginBottom: "-1px"
          }}>
        ...
        {activeTab === "staff" && (
          <button onClick={() => setShowAddStaff(true)} style={{ 
            ...
            fontSize: "0.75rem", border: "1px solid var(--brand)", marginBottom: "0.25rem",
            color: "var(--brand)", background: "transparent", borderRadius: "12px", fontWeight: "700", cursor: "pointer", transition: "all 0.2s" 
          }}>
      ...
      {loading ? (
        ...
        <div className="table-container">
          <table className="custom-table">
            <thead>
              <tr><th>Name</th><th>Phone</th><th>Login Code</th><th>Outlet</th><th>Status</th><th>Actions</th></tr>
            </thead>
            <tbody>
              ...
              {users.filter(u => u.role === "staff").length === 0 && <tr><td colSpan="6" style={{ textAlign: "center", padding: "2rem" }}>No staff found for your outlets.</td></tr>}
            </tbody>
          </table>
        </div>
      ) : (
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  showToast("Stock request submitted to Kitchen!", "success");
                                } catch (e) {
        ...
        <form onSubmit={handleAddStaff} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          <div className="form-group" style={{ margin: 0 }}>
            ...
            <input
              ...
              onChange={e => setAddStaffForm({ ...addStaffForm, pin: e.target.value.replace(/\D/g, "").slice(0, 4) })}
            />
          </div>
          <div className="form-group" style={{ margin: 0 }}>
            <label className="form-label">Full Name</label>
